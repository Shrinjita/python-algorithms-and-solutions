...
def reg_server(root,frame1,name,sex,aadhar,city,passw):
    if(passw=='' or passw==' '):
        msg = Message(frame1, text="Error: Missing Fileds", width=500)
        msg.grid(row = 10, column = 0, columnspan = 5)
        return -1

    vid = df.taking_data_voter(name, sex, aadhar, city, passw)
    for widget in frame1.winfo_children():
        widget.destroy()
    if vid==-1:
        txt = "Invalid Aadhar or Already Registered"
...
def Register(root,frame1):
    ...
    Label(frame1, text="Aadhar Number:           ", anchor="e", justify=LEFT).grid(row = 5,column = 0)
    ...
    e5 = Entry(frame1, textvariable = aadhar).grid(row = 5, column = 2)
    ...
    reg = Button(frame1, text="Register", command = lambda: reg_server(root, frame1, name.get(), sex.get(), aadhar.get(), city.get(), password.get()), width=10)
